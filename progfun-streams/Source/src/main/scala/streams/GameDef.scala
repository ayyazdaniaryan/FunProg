...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      ...
      def allNeighbors(allMoves: List[Move]): List[(Block,Move)] = allMoves.head match{
        case List() => Nil
      	case Up => (currBlock.up,Up) :: allNeighbors(allMoves.tail)
      	case Down => (currBlock.down,Down) :: allNeighbors(allMoves.tail)
      	case Left => (currBlock.left,Left) :: allNeighbors(allMoves.tail)
      	case Right => (currBlock.right,Right) :: allNeighbors(allMoves.tail)
      }
      allNeighbors(moves)
      */
    }
