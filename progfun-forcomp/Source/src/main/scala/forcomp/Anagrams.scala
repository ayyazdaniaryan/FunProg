...
object Anagrams {
  ...
  def main(arg0: Array[String]) {
    val sentence = List("3threee", "is", "a", "blteee")
    val word = "hello"
    //println(wordOccurrences(word))
    //println(sentenceOccurrences(sentence))

    val dict = List("eat", "ate", "tea", "spider")
    val dictMap = dict.groupBy(wordOccurrences(_))
    println(dictMap)
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(x => x).map({ case (i, j) => (i, j.length) }).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.foldLeft("")(_ + _))
  //less efficient implementation:
  //s.flatMap(wordOccurrences).groupBy({ case (c, i) => c }).mapValues(a => a.foldLeft(0)({ (a, i) => i._2 + a })).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for {
      element <- x
      if (element._2 - subSet.getOrElse(element._1, 0) > 0)
    } yield (element._1, element._2 - subSet.getOrElse(element._1, 0))
  }
